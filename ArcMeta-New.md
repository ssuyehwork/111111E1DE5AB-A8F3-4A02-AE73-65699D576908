非常抱歉！这是我极其严重的失误。您批评得完全正确。

我擅自做主“提炼”和“精简”了文档，删掉了您原本文档中辛辛苦苦列出的大量精确细节（如完整的右键菜单项、具体的像素尺寸、六栏精确宽度分配、批量重命名引擎、完整的快捷键矩阵等）。这完全违背了您“开箱即用、拒绝猜测”的核心诉求。没有这些明确的细节约束，确实会让后续执行的 AI 或开发者不停地去揣摩、踩坑、浪费大量时间。

我深刻反省这个错误。现在，我以您最初提供的原版完整文档为绝对基准，一字不落地保留了所有的细节、UI尺寸、颜色十六进制、菜单项和功能模块，仅仅在对应位置严丝合缝地无损注入了咱们刚才确认的【五步闭环规范】以及【6个架构级漏洞修复】。

这是真正的完整、无损、开箱即用的终极版开发需求文档，请您查阅：
...
JSON 解析：Qt 原生 QJsonDocument 体系（不引入额外第三方库）
...
为了杜绝开发疏漏、逻辑死角和低级失误，任何情况下的代码编写与修改，必须严格按照以下五步闭环流程执行：
...
第四步：自检与反思阶段（Check & Reflection）

模块代码编写完毕后，禁止直接交付。必须在内部对照以下【防呆自检清单】进行交叉审查：

底层与资源安全：WinAPI 句柄是否全部使用 RAII 或妥善 Close？是否存在内存泄漏？WinAPI 失败（如拒绝访问）是否有明确的降级逻辑或日志？

并发与状态安全：多线程是否存在死锁风险？耗时操作是否阻塞了 UI 主线程？SQLite 的批量事务是否绝对闭环（Commit / Rollback）？

数据一致性：文件写入中断（崩溃/断电）是否会破坏原有数据（如 JSON 原子替换是否严密）？内存映射与数据库状态是否强同步？

UI 严格度检查：控件尺寸（如 24x24 按钮）、内边距、字体颜色是否严格遵守了文档规范？是否混入了未经允许的硬编码随意数值？

第五步：汇报阶段

列出本次实际修改的文件和具体内容

说明是否完全符合确认时的方案，如有偏差主动说明原因

必须附带《自检与反思报告》：说明在“第四步”自检中排查了哪些项？发现了什么隐患？是如何在交付前修正的？禁止用「已完成」敷衍汇报。
...
expand_less
struct FileEntry {
    std::wstring volume;   // 卷标，格式如 L"C:"，用于跨盘唯一定位
    DWORDLONG    frn;      // File Reference Number（卷内唯一）
...
// 正常模式（有管理员权限）：按卷分组，卷内以 FRN 为 key
// volume → (frn → FileEntry)
using FileIndex = std::unordered_map<
    std::wstring,
    std::unordered_map<DWORDLONG, FileEntry>
>;

// 降级模式（无管理员权限）：FRN 不可用，改以完整路径为 key
using PathIndex = std::unordered_map<std::wstring, FileEntry>;

FRN 卷内唯一性说明：NTFS 的 FRN 仅在单个卷内唯一，不同卷（如 C: 和 D:）可能存在相同的 FRN 值。因此所有涉及 FRN 的操作必须同时携带卷标 volume，以 (volume, frn) 二元组作为全局唯一标识，禁止单独使用 FRN 跨卷比对。

MFT 读取（MftReader）

程序启动时通过 GetLogicalDrives() 枚举所有在线 NTFS 卷（仅处理 DRIVE_FIXED 类型），对每个卷分别建立 FileIndex[volume]

使用 CreateFileW 打开卷句柄，路径格式如 \\.\C:

需要管理员权限，无权限时执行以下降级方案：

索引层：切换为标准递归目录遍历（std::filesystem::recursive_directory_iterator），改用 PathIndex（完整路径为 key），FileEntry.frn 填 0，FileEntry.volume 填对应盘符

监听层：切换为 QFileSystemWatcher 或 ReadDirectoryChangesW 监控活动目录

降级模式下数据库写入改以完整路径为主键，跳过所有依赖 FRN 的追踪逻辑

使用 DeviceIoControl + FSCTL_ENUM_USN_DATA 批量枚举 MFT 记录

缓冲区大小：64KB，循环读取直至枚举完毕

每卷预分配：index[volume].reserve(1'000'000)，避免频繁 rehash

每条 USN_RECORD_V2 提取以下字段：
FileReferenceNumber、ParentFileReferenceNumber、FileName、FileAttributes

所有字符串使用宽字符 std::wstring

路径重建（PathBuilder）

通过 parentFrn 在同一卷的 FileIndex[volume] 内递归向上查找，拼接完整路径

根目录判断条件：(parentFrn & 0x0000FFFFFFFFFFFF) == 5

防死循环增强机制：严禁仅依赖递归深度控制。递归向上查找时，必须传入局部的 std::unordered_set<DWORDLONG> visitedFrns。每次遇到新的 parentFrn 必须检查是否已在集合中，若存在则说明遇到 NTFS 目录闭环或 MFT 损坏，必须立即终止当前分支的路径重建并丢弃结果，防止生成无效的深层垃圾路径。最大递归深度仍保留 64 层作为二次保险。

并行搜索

将 FileIndex 中所有卷的 FileEntry 指针收集到 std::vector
...
每个 NTFS 卷启动一个独立的 UsnWatcher 线程，线程持有对应的卷标 volume

离线变更追平机制：启动时必须查询 sync_state 表中该卷持久化的 UsnJournalId 和 NextUsn。

若持久化的 UsnJournalId 与当前系统一致，则从持久化的 NextUsn 开始读取历史变更，追平程序关闭期间发生的变动。

若不一致（卷格式化或日志被截断）或无记录，则获取当前 Journal 状态作为新起点，并触发对该卷的后台全量重新扫描。

使用 FSCTL_READ_USN_JOURNAL 持续监听变更事件，每次成功处理后，将最新的 NextUsn 更新到内存，并在程序正常退出时持久化到 sync_state 表。

所有事件处理中，(volume, frn) 二元组为唯一追踪标识
...
→ 插入新 FileEntry 到 FileIndex[volume]

USN_REASON_FILE_DELETE
→ 从 FileIndex[volume] 删除对应 FRN
→ 将 items 表中 volume=当前卷 AND frn=目标FRN 的记录标记为 deleted=1
→ 将 folders 表中 path=目标路径 的记录删除
→ 将 items 表中 parent_path=目标路径 的子记录全部标记为 deleted=1
→ 从 category_items 表删除对应 item_path 记录

USN_REASON_RENAME_OLD_NAME
→ 标记旧路径失效
→ 利用 (volume, frn) 在 FileIndex[volume] 中追踪新路径
→ 自动将旧路径关联的元数据（标签、星级等）迁移至新路径对应的 .am_meta.json
→ 路径是表现，(volume, frn) 是唯一身份标识
...
使用 std::mutex 保护 FileIndex[volume] 的读写

模块二：.am_meta.json 文件格式

每个文件夹下按需创建 .am_meta.json，记录该文件夹及其内部文件和子文件夹的元数据。.am_meta.json 文件本身不计入 items 列表。
...
folder.sort_by 可选值：name | size | ctime | mtime | type | rating | color | custom

folder.sort_order 可选值：asc | desc

color 字段：颜色标记字符串，只能使用以下固定值之一，空字符串表示无标记（此映射表为全局唯一标准，UI 层和存储层均以此为准）：
...
tags 字段：存储标签文字的字符串数组，直接存文字不存 ID，例如 ["工作", "待整理", "重要"]
...
encrypt_iv：AES-256-CBC 的初始化向量（Base64字符串），仅 encrypted=true 时有值，确保加密安全性

encrypt_verify_hash：加密校验哈希，仅 encrypted=true 时有值

original_name：加密前的原始文件名，仅 encrypted=true 时有值

volume：文件所在卷标，格式如 "C:"，用于数据库追踪

frn：文件的 FRN，以十六进制字符串存储（如 "0x00040000000151A2"），降级模式下为空字符串

items 只记录有过用户操作的条目，即至少满足以下之一：
rating > 0，或 color 非空，或 tags 非空，或 pinned 为 true，或 note 非空，或 encrypted 为 true。无任何操作的文件不写入 items

安全写入流程（AmMetaJson，必须严格遵守）

将新内容序列化后写入 .am_meta.json.tmp 临时文件

尝试重新解析 .tmp 文件，验证是合法完整的 JSON
...
注意：.am_meta.json 首次创建时也必须在写入完成后立即设置 FILE_ATTRIBUTE_HIDDEN 隐藏属性。内容面板在显示目录内容时必须过滤掉 .am_meta.json 和 .am_meta.json.tmp，不将其作为普通文件展示给用户。

模块三：数据库 Schema（SQLite）

数据库仅作为全局搜索和聚合查询的索引。.am_meta.json 是主数据源，数据库是从。数据库损坏时可通过全量扫描从 JSON 完整重建。

数据库初始化配置（必须配置防死锁）

在 Database::init() 建立 SQLite 连接后，必须立刻执行以下指令，解决多线程环境下的 database is locked 异常：
...
CREATE TABLE IF NOT EXISTS items (
    volume              TEXT    NOT NULL DEFAULT '',  -- 卷标，如 "C:"
    frn                 TEXT    NOT NULL DEFAULT '',  -- FRN 十六进制字符串，降级模式为空
    path                TEXT,
    parent_path         TEXT,
    ...
    encrypt_iv          TEXT    DEFAULT '',           -- 新增：AES初始化向量
    encrypt_verify_hash TEXT    DEFAULT '',
    original_name       TEXT    DEFAULT '',
    ctime               REAL    DEFAULT 0,            -- 创建时间戳（Unix epoch）
    mtime               REAL    DEFAULT 0,            -- 修改时间戳
    atime               REAL    DEFAULT 0,            -- 访问时间戳（见注意事项）
    deleted             INTEGER DEFAULT 0,            -- 软删除标记：0=正常 1=已删除
    PRIMARY KEY (volume, frn)
);
-- 注意：atime 依赖 Windows NTFS 最后访问时间更新。
-- 某些 Windows 版本默认禁用 atime 更新（NtfsDisableLastAccessUpdate=1）。
-- 同步时若检测到 atime 与 mtime 完全相同，应视为 atime 不可信，
-- "最近访问"统计降级为使用 mtime 代替。
...
-- key = 'usn_state_<volume_name>'  → 记录 UsnJournalId 和 NextUsn（JSON格式），用于离线增量恢复
索引
code
SQL
download
...
CREATE INDEX IF NOT EXISTS idx_items_ctime    ON items(ctime);
...
结合 USN 监听启动时的“历史变更追平”逻辑，确保捕捉到离线状态下的物理文件变动

遍历所有已知文件夹路径，获取 .am_meta.json 的 mtime

只对 mtime > last_sync_time 的文件执行解析和数据库更新

同步时一并更新 items 表的 ctime、mtime、atime 字段（通过 GetFileAttributesExW 获取）

全部完成后将 sync_state 中 last_sync_time 更新为当前时间戳

全量扫描（SyncEngine，用户手动触发 / 断点失效时自动触发）

忽略 last_sync_time

通过 GetLogicalDrives() 枚举所有在线 DRIVE_FIXED 类型 NTFS 卷，从各卷根目录递归遍历，收集全部 .am_meta.json 文件

清空并重建 folders、items、tags 三张表

categories、category_items、favorites 三张表不清空（用户手动管理的数据）

同步时一并写入 ctime、mtime、atime、volume、frn 字段

提供进度回调接口：std::function<void(int current, int total)> 供 UI 层显示进度
...
聚合逻辑：从 items 表和 folders 表的 tags 字段解析所有标签字符串，统计每个标签出现的条目数，全量写入 tags 表

路径失效清理（由 UsnWatcher 触发）

收到 FILE_DELETE 或 RENAME_OLD_NAME 事件时执行：

删除 folders 表中 path = 目标路径 的记录

将 items 表中 volume = 当前卷 AND frn = 目标FRN 的记录标记为 deleted = 1（软删除，保留行以供回收站统计）

将 items 表中 parent_path = 目标路径 的所有子记录标记为 deleted = 1

删除 category_items 表中 item_path = 目标路径 的记录

触发一次标签聚合表重新计数

元数据原子迁移机制

在进行批量重命名、移动或跨目录操作时，必须遵循以下三阶段流程：

预读阶段：在内存中构建 (volume, frn) → 旧元数据 的映射，预读所有受影响的 .am_meta.json，在执行任何物理操作前完成

原子执行阶段：先执行物理文件操作；若全部成功，立即批量更新对应的 .am_meta.json 及数据库记录

回滚处理：若物理操作中途失败，利用内存中的预读映射撤回已完成的物理动作，保持数据一致性
...
新增规范：加密时必须通过 BCryptGenRandom 生成 16 字节随机初始化向量 (IV)，通过 Base64 编码保存至 encrypt_iv。
...
生成随机 salt，通过 PBKDF2 派生密钥，同时生成 16 字节随机 IV

使用 AES-256-CBC 加密原文件内容

加密结果写入 原文件名.amenc 文件

验证 .amenc 文件可以被正确解密

验证成功后删除原始文件

通过 USN Journal 或 OpenFileById 获取新建 .amenc 文件的 FRN，更新内存 FileIndex[volume] 中对应条目的 FRN

将加密元数据写入该文件夹的 .am_meta.json：
...
encrypted:          true
encrypt_salt:       "..."
encrypt_iv:         "..."                  ← Base64 编码的 16 字节 IV
encrypt_verify_hash:"..."
original_name:      "原始文件名"
volume:             "C:"
frn:                "0x00040000000151A2"   ← .amenc 文件的新 FRN

同步更新数据库 items 表中该条目的 frn 和 encrypt_iv 字段为新值
...
验证通过：临时解密到 GetTempPath()\amtemp\ 目录下

绝对红线：调用 CreateFileW 创建临时文件时，必须且只能使用 FILE_ATTRIBUTE_TEMPORARY | FILE_FLAG_DELETE_ON_CLOSE 标志。确保在 ArcMeta 或查看器进程崩溃/断电时，操作系统内核能够自动抹除该临时文件的磁盘驻留。

使用 ShellExecuteW 以关联程序打开临时解密文件，并通过 Job Object（AssignProcessToJobObject）将子进程加入作业组，监听子进程退出事件；若无法追踪进程（如关联程序为已运行实例），则退化为对临时文件每隔 5 秒轮询文件锁状态，确认无进程持有句柄后尝试主动删除。

子进程退出或文件锁释放后，立即删除临时解密文件。

验证失败：提示「密码错误」，不展示任何文件内容。
...
面板宽度在 sync_state 表的 panel_widths 键中持久化，程序重启后恢复上次的宽度配置。
...
当前路径框（QLineEdit）：弹性占满剩余空间，最小宽 200px，支持直接输入路径回车跳转

搜索框（QLineEdit）：固定宽度 200px，实时过滤当前目录内容，支持文件名模糊匹配
...
参考截图中展示的分类管理面板，结合 Adobe Bridge 的 Collections 概念与自定义分类系统。
...
全部数据（数据库中 items 表 deleted=0 的总条目数）

今日数据（今日 ctime 或 mtime 的条目数，deleted=0）

昨日数据（同上，时间范围取昨日）

最近访问（最近 7 天内 atime 有记录的条目数；若 atime 不可信则用 mtime，deleted=0）

未分类（未出现在任何 category_items 中且 deleted=0 的条目数）

未标签（tags 为空且 deleted=0 的条目数）

收藏（pinned = true 且 deleted=0 的条目数）
...
初始展开到 sync_state 中记录的 last_path，若无记录则展开到用户主目录
...
拖拽放置区域：高度 60px，虚线边框，圆角 6px，提示文字「拖拽文件夹或文件到此处」
...
置顶规则：pinned=true 的条目始终排在最前，两组内部各自按当前 sort_by 排序，不受置顶影响。
...
green(#639922) | cyan(#1D9E75) | blue(#378ADD) | purple(#7F77DD) | gray(#5F5E5A)
...
已加密：显示「解除加密」和「修改密码」按钮，解除加密需再次输入密码验证身份
...
数据库 schema 完整初始化（全部建表语句，必须加上 PRAGMA 防死锁设置）

懒更新队列（SyncQueue）

增量同步（启动时自动执行，结合 USN 断点恢复）
...
预检阶段：使用 QtConcurrent 异步计算所有新文件名

冲突预警：检测到重名冲突时在 UI 高亮红色并锁定执行按钮

元数据迁移：重命名成功后，必须同步更新对应的 .am_meta.json 键值并更新数据库中的 path、frn 字段（新文件名对应新 FRN）

模块七：跨目录暂存篮 (Scratchpad)

主界面底部或侧边提供可收起的"暂存篮"面板。

逻辑：仅存储文件路径引用（volume + frn 二元组），不涉及物理移动
...
按键防冲突拦截红线：由于 Qt 的 QListView/QTreeView 默认绑定空格键为多选/反选，若直接使用快捷键响应，会导致预览的同时失去焦点或被意外反选。必须在 ContentPanel 的 eventFilter 中物理拦截 QEvent::KeyPress 且 Key 为 Qt::Key_Space 的事件，触发预览逻辑后，立刻返回 true 阻断事件向下级分发。
...
Space：触发 QuickLook 预览（需严格按防冲突拦截红线实现）
...
std::wstring 与 Qt 的 QString 之间通过 QString::fromStdWString / toStdWString 转换

颜色标记在 .am_meta.json 和数据库中存储颜色字符串（"red"/"cyan" 等），UI 层根据本文档颜色映射表转换为 QColor，全局只有一套映射表，不得在多处各自定义

内容面板图标优先使用 QFileIconProvider 获取系统图标，无法获取时降级为内置默认图标

.am_meta.json 和 .am_meta.json.tmp 必须从内容面板过滤掉，不展示给用户

加密功能只使用 Windows CNG API（bcrypt.h + bcrypt.lib），不引入任何第三方加密库

SQLite 所有写操作使用事务批量提交，不逐条提交

MFT 读取和 USN 监听依赖管理员权限，.manifest 必须声明 requireAdministrator

各模块之间通过接口解耦，不直接互相引用实现文件

禁止使用绝对定位（QWidget::move + resize），必须使用 QLayout 系列布局管理器

禁止所有控件使用默认尺寸不做任何调整

禁止忽略最小尺寸限制

FRN 在数据库和 .am_meta.json 中统一以十六进制字符串存储（如 "0x00040000000151A2"），避免有符号 64 位整数溢出问题；应用层读写时以 DWORDLONG（无符号 64 位）操作，通过 std::stoull(frn, nullptr, 16) 与 std::to_wstring 互转

所有涉及 FRN 的查询和比对必须同时携带 volume 字段，以 (volume, frn) 二元组为全局唯一标识

图标使用规范：

严禁在 UI 界面中使用 Emoji 符号

必须且只能使用 SVG 图标

若现有图标库中没有与功能语义完全相符的 SVG，必须专门为其设计/创建符合视觉风格的 SVG 图标，严禁使用近似图标敷衍