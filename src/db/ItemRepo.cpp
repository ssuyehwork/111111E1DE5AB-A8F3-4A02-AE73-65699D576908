...
bool ItemRepo::save(const std::wstring& parentPath, const std::wstring& name, const ItemMeta& meta) {
    ...
    QSqlQuery q(db);
    q.prepare("INSERT OR REPLACE INTO items (volume, frn, path, parent_path, type, rating, color, tags, pinned, note, encrypted, encrypt_salt, encrypt_iv, encrypt_verify_hash, original_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    q.addBindValue(meta.encrypted ? 1 : 0); q.addBindValue(QString::fromStdString(meta.encryptSalt));
    q.addBindValue(QString::fromStdString(meta.encryptIv)); q.addBindValue(QString::fromStdString(meta.encryptVerifyHash));
...
bool ItemRepo::saveBasicInfo(const std::wstring& volume, const std::wstring& frn, const std::wstring& path, const std::wstring& parentPath, bool isDir, double mtime, double size) {
    ...
    q.addBindValue(isDir ? "folder" : "file"); q.exec();
    QSqlQuery u(db);
    ...
    u.addBindValue(mtime); u.addBindValue(QString::fromStdWString(volume)); u.addBindValue(QString::fromStdWString(frn));
    Q_UNUSED(size); return u.exec();
}
...
QStringList ItemRepo::searchByKeyword(const QString& keyword, const QString& parentPath) {
    ...
    QSqlDatabase db = ArcMeta::Database::instance().getThreadDatabase();
    QSqlQuery q(db);
    if (parentPath.isEmpty()) { q.prepare("SELECT path FROM items WHERE path LIKE ? AND deleted = 0 LIMIT 300"); q.addBindValue("%" + keyword + "%"); }
    else { q.prepare("SELECT path FROM items WHERE parent_path = ? AND path LIKE ? AND deleted = 0 LIMIT 300"); q.addBindValue(parentPath); q.addBindValue("%" + keyword + "%"); }
    QStringList results; if (q.exec()) { while (q.next()) results << q.value(0).toString(); }
    return results;
}
