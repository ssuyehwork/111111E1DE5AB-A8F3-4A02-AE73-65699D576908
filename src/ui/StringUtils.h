...
public:
    ...
    static void copyToClipboard(const QString& content) {
     ...
     */
    static void copyItemsToClipboard(const QList<QVariantMap>& items) {
        if (items.isEmpty()) return;
        if (items.size() == 1) {
            QString content = items.first().value("content").toString();
            copyToClipboard(content);
        } else {
            QStringList texts;
            for (const auto& item : items) texts << htmlToPlainText(item.value("content").toString());
            ClipboardMonitor::instance().skipNext();
     ...
     */
    static QString generatePreviewHtml(const QString& title, const QString& content, const QString& type, const QByteArray& data, double zoomFactor = 1.0) {
