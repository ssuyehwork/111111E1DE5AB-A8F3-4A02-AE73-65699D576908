...
void MetaPanel::initUi() {
    ...
    // 2026-04-12 按照用户要求：保持 5 像素的边缘间距，解决太贴近边缘的问题
    ...
    // 2026-04-12 修正：增加 16px 间距，消除分割线与下方方框“黏在一起”的视觉 Bug
    m_containerLayout->addSpacing(16);
    ...
    m_containerLayout->addWidget(createSectionBox("edit", "备注", m_noteEdit));
    m_containerLayout->addSpacing(12);

    // --- 红色圈着部分的物理还原：标签容器 ---
    m_tagContainer = new QWidget(this);
    m_tagFlowLayout = new FlowLayout(m_tagContainer, 0, 8, 8); // 增加间距使其更具“呼吸感”
    m_containerLayout->addWidget(createSectionBox("tag", "标签", m_tagContainer));
    m_containerLayout->addSpacing(12);

    // --- 物理还原：底部的药丸状标签输入框 ---
    m_tagEdit = new QLineEdit(m_container); 
    m_tagEdit->setPlaceholderText("输入标签添加... (双击更多)"); 
    m_tagEdit->setFixedHeight(32); 
...
void MetaPanel::addInfoRow(const QString& label, QLabel*& valueLabel) {
    ...
    // 2026-04-12 修正：由脑残的右对齐改为整齐的左对齐，并设定字段宽度
    QHBoxLayout* rl = new QHBoxLayout(row); 
    rl->setContentsMargins(10, 6, 10, 6); 
    rl->setSpacing(10);

    QLabel* kl = new QLabel(label, row); 
    kl->setFixedWidth(65); // 设定固定宽度以对齐数值列
    kl->setStyleSheet("font-size: 11px; color: #888888;");
    rl->addWidget(kl);

    valueLabel = new QLabel("-", row); 
    valueLabel->setWordWrap(true); 
    valueLabel->setStyleSheet("font-size: 12px; color: #EEEEEE; font-weight: 500;");
    valueLabel->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    rl->addWidget(valueLabel, 1); // 给数值 Label 设置 stretch 比例，确保长文本换行
...
QWidget* MetaPanel::createSectionBox(const QString& iconName, const QString& title, QWidget* content) {
    // 2026-04-12 修正：圆角由 12px 统一调整为 6px，对齐专业 UI 规范
    ...
    QLabel* iconLbl = new QLabel(box);
    iconLbl->setPixmap(UiHelper::getIcon(iconName, QColor("#888888"), 16).pixmap(16, 16));
    iconLbl->setStyleSheet("background: transparent; border: none;");
    header->addWidget(iconLbl);

    QLabel* titleLbl = new QLabel(title, box);
    titleLbl->setStyleSheet("font-size: 11px; font-weight: bold; color: #888888; background: transparent; border: none; text-transform: uppercase;");
