...
TagPill::TagPill(const QString& text, QWidget* parent)
    ...
    m_closeBtn->setStyleSheet(
        "QPushButton { border: none; background: transparent; color: #B0B0B0; font-size: 14px; font-weight: bold; }"
...
void StarRatingWidget::paintEvent(QPaintEvent*) {
    ...
    painter.setFont(f);
    for (int i = 0; i < 5; ++i) {
        painter.setPen((i < m_rating) ? QColor("#EF9F27") : QColor("#444444"));
        painter.drawText(QRect(i * 24, 0, 20, 20), Qt::AlignCenter, (i < m_rating) ? "★" : "☆");
    }
}
...
ColorPickerWidget::ColorPickerWidget(QWidget* parent) : QWidget(parent) {
    ...
    setFixedSize(m_colors.size() * 18 + (m_colors.size() - 1) * 6, 18);
    setCursor(Qt::PointingHandCursor);
}
