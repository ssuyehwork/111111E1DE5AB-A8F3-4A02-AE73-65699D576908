...
protected:
    bool filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const override {
        ...
        // 2026-04-12 深度修复：由于基类过滤逻辑在特定模型下可能产生排斥且会自动转义标点符号，
        // 此处改为手动维护的 m_searchQuery 提取文件名进行精准的关键词包含检查。
        if (m_searchQuery.isEmpty()) return true;

        // 获取当前行的显示文本（列 0）
        QString fileName = idx.data(Qt::DisplayRole).toString();
        // 执行不区分大小写的包含检查
        bool match = fileName.contains(m_searchQuery, Qt::CaseInsensitive);

        if (!match) {
            static int rejectCount = 0;
            if (rejectCount++ % 10 == 0) { // 采样输出，防止日志淹没
                qDebug() << "[Filter] 项被排除 (关键词不匹配):" << fileName << " 关键词:" << m_searchQuery;
            }
        }
...
void GridItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    // 2026-04-12 按照用户要求：修复英文长文件名无法换行的问题。
    // 原因：Qt 的 TextWordWrap 只在空格和连字符处断行，下划线与点号不被视为合法断点。
    // 修复方案：在 '_' 和 '.' 字符后注入零宽空格（U+200B），
    // 零宽空格在渲染时完全不可见，但让 Qt 的文字引擎识别此处为合法断行位置。
    ...
    painter->drawText(nameRect.adjusted(4, 0, -4, 0), Qt::AlignCenter | Qt::TextWordWrap, displayName);
