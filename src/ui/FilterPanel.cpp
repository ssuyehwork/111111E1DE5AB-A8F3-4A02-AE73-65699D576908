...
void FilterPanel::addCheckboxFilter(QVBoxLayout* layout, const QString& label, int count, const QColor& iconColor) {
    ...
    if (label.contains("★")) {
        QWidget* starContainer = new QWidget(r);
        QHBoxLayout* sl = new QHBoxLayout(starContainer);
        sl->setContentsMargins(0, 0, 0, 0);
        sl->setSpacing(2);
        int starCount = label.count("★");
        for (int i = 0; i < starCount; ++i) {
            QLabel* s = new QLabel(starContainer);
            s->setPixmap(UiHelper::getPixmap("star_filled", QSize(12, 12), QColor("#EF9F27")));
            sl->addWidget(s);
        }
        if (label.contains("及以上")) {
            QLabel* sText = new QLabel(" 及以上", starContainer);
            sText->setStyleSheet("font-size: 11px; color: #5F5E5A;");
            sl->addWidget(sText);
        }
        rl->addWidget(starContainer);
    } else {
        QLabel* lt = new QLabel(label, r);
        lt->setStyleSheet("font-size: 12px; color: #EEEEEE;");
        rl->addWidget(lt);
    }
