...
void MainWindow::initUi() {
    ...
    connect(m_contentPanel, &ContentPanel::requestQuickLook, [this](const QString& path) {
        m_currentQuickLookPath = path;
        QuickLookWindow::instance().previewFile(path);
    });

    // 2026-04-11 按照用户要求：双向联动，实现预览窗内方向键切图导航
    connect(&QuickLookWindow::instance(), &QuickLookWindow::prevRequested, [this]() {
    ...
    connect(&QuickLookWindow::instance(), &QuickLookWindow::nextRequested, [this]() {
    ...
    connect(&QuickLookWindow::instance(), &QuickLookWindow::colorRequested, [this](const QString& color) {
        if (m_currentQuickLookPath.isEmpty()) return;
    ...
    connect(m_filterPanel, &FilterPanel::resetSearchRequested, [this]() {
    ...
    // 历史面板信号对接
    connect(m_searchHistoryPanel, &SearchHistoryPanel::historyItemClicked, [this, performSearch](const QString& keyword) {
        m_searchEdit->setText(keyword);
        performSearch(keyword);
    });

    connect(m_searchHistoryPanel, &SearchHistoryPanel::historyItemRemoved, [this](const QString& keyword) {
        m_searchHistory.removeAll(keyword);
        QSettings settings("ArcMeta", "ArcMeta");
        settings.setValue("Search/History", m_searchHistory);
        m_searchHistoryPanel->setHistory(m_searchHistory);
    ...
    connect(m_searchHistoryPanel, &SearchHistoryPanel::clearAllRequested, [this]() {
...
void MainWindow::initToolbar() {
    ...
    m_btnSearchMode = new QPushButton("局部", m_searchContainer);
    m_btnSearchMode->setFixedSize(50, 32);
    m_btnSearchMode->setProperty("tooltipText", "点击切换：局部(当前目录) / 全局(数据库)");
    m_btnSearchMode->installEventFilter(this);
    m_btnSearchMode->setStyleSheet(
        "QPushButton {"
        "  background: #282828; border: 1px solid #444444;"
        "  border-left: none;"
    ...
    connect(m_btnSearchMode, &QPushButton::clicked, [this]() {
        QMenu modeMenu(this);
        modeMenu.setStyleSheet(
            "QMenu { background: #2D2D2D; color: #EEE; border: 1px solid #444; padding: 4px; border-radius: 8px; }"
            "QMenu::item { padding: 6px 18px 6px 10px; border-radius: 4px; font-size: 12px; }"
            "QMenu::item:selected { background: #3E3E42; }"
        );
        QAction* actLocal  = modeMenu.addAction("局部搜索");
        QAction* actGlobal = modeMenu.addAction("全局搜索");
        QAction* res = modeMenu.exec(m_btnSearchMode->mapToGlobal(QPoint(0, m_btnSearchMode->height() + 2)));
        if (res == actLocal)  { m_searchMode = LocalSearch;  m_btnSearchMode->setText("局部"); }
        if (res == actGlobal) { m_searchMode = GlobalSearch; m_btnSearchMode->setText("全局"); }
    });

    searchLayout->addWidget(m_searchEdit, 1);
    searchLayout->addWidget(m_btnSearchMode);
}
