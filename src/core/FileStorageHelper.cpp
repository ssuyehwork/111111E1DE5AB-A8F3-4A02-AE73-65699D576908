...
int FileStorageHelper::parseCsvFile(const QString& csvPath, int catId, QList<int>* createdNoteIds) {
    ...
    if (rows.size() > 1) {
        ...
        for (int i = 1; i < rows.size(); ++i) {
            ...
            int noteId = DatabaseManager::instance().addItem(title, content, tagsStr.split(",", Qt::SkipEmptyParts), "", catId);
            if (noteId > 0) {
...
bool FileStorageHelper::storeFile(const QString& path, int categoryId, 
    ...
    if (ok) {
        ...
        int noteId = DatabaseManager::instance().addItem(
            title,
            relativePath,
            {"导入文件"},
            "#2c3e50",
    ...
    std::function<void(const QJsonObject&, int)> importCategory = [&](const QJsonObject& obj, int parentId) {
        ...
        if (newCatId > 0) {
            ...
            for (const auto& v : notes) {
                ...
                if (noteId > 0) {
                    // 恢复状态属性
                    db.updateItemState(noteId, "rating", n["rating"].toInt());
                    db.updateItemState(noteId, "is_pinned", n["is_pinned"].toInt());
                    db.updateItemState(noteId, "is_favorite", n["is_favorite"].toInt());
                }
            }
